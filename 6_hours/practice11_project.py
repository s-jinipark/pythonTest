...
class Tank(AttackUnit):
    ...
    def set_seize_mode(self):
        if Tank.seize_developed == False: # 시즈모드 개발이 안됬으면 그냥 나감
            return

        # 현재 시즈모드가 아닐 때 -> 시즈모드
        #if Tank.seize_mode == False: # 오류 : Class 'Tank' has no 'seize_mode' member
...
class Wraith(FlyableAttackUnit):
    ...
    def clocking(self):
        if self.clocked == True: # 클로킹 모드 -> 모드 해제
            print("{0} : 크로킹 모드 해제합니다.".format(self.name))
            self.clocked = False
        else: # 클로킹 모드 해제 -> 모드 설정
            print("{0} : 크로킹 모드 설정합니다.".format(self.name))
            self.clocked = True
