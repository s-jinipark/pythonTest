...
class HairShop(Shop):   #HairShop@@@:
    def __init__(self):
        super().__init__()
        
    def reserve(self, customer):   #@@@:
        if customer.num_of_people != 1:   #@@@ != 1:
            return False
...
class Restaurant(Shop):   #Restaurant@@@:
    def __init__(self):
        super().__init__()
        
    def reserve(self, customer):   #@@@:
        if customer.num_of_people < 2 or customer.num_of_people >8 :  #if @@@:
            return False
        count = 0
        for r in self.reserve_list:
            #print(r)
            if customer.time == r.time:  # if @@@:
                count += 1  # 2 팀까지 겹칠 수 있음
        if count >= 2:
            return False
        self.reserve_list.append(customer)
        return True
