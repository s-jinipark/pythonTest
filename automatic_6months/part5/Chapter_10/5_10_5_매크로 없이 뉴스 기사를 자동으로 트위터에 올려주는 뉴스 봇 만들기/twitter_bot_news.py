...
class NewsBot:
    ...
    def login(self, id, ps):
        ...
        #id_input = self.driver.find_element_by_name("session[username_or_email]")
        # 2022 0218
        id_input = self.driver.find_element_by_name("text")
        # id를 입력합니다.
        id_input.send_keys(id)

        # 2022 0218 (2page 로 나눠짐)
        ...
        #ps_input = self.driver.find_element_by_name("session[password]")
        # 2022 0218
        time.sleep(3)  #  -> 화면 구성의 시간이 필요합
        ps_input = self.driver.find_element_by_name("password")
    ...
    def prepare_contents(self, keyword):
        ...
        # 2022 0218
        news_elements = self.driver.find_elements_by_class_name("ftSUBd")
        # 모든 엘레멘트로부터 정보를 추출하겠습니다.
        for el in news_elements:
            # 기사 제목을 추출합니다. <JheGif.nDgy9d> 태그로 작성되었습니다.
            headline = el.find_element_by_class_name("mCBkyc.y355M.JQe2Ld.nDgy9d").text
            # 기사 하이퍼링크 태그를 추출합니다.
            hyperlink = el.find_element_by_tag_name("a")
            # 기사 하이퍼링크 태그에서 기사 주소를 추출합니다.
            news_url = hyperlink.get_attribute("href")
            ...
            reference = 'test'
            # 뉴스 앞 부분을 추출해 냅니다. "Y3v8qd"라는 클래스로 기록되어 있습니다.
            head = el.find_element_by_class_name("GI74Re.nDgy9d").text
            # 트윗에 올릴 기사 요약을 만듭니다.
            news_summary = "\n".join((headline, reference, head, self.endswith, news_url))
            self.contents.append(news_summary)
    ...
    def tweet(self, string):
        ...
        board = self.driver.find_element_by_xpath('//*[@id="layers"]/div[2]/div/div/div/div/div/div[2]/div[2]/div/div/div/div[3]/div/div[1]/div/div/div/div/div[2]/div[1]/div/div/div/div/div/div/div/div/div/label/div[1]/div/div/div/div/div[2]/div/div/div/div')
